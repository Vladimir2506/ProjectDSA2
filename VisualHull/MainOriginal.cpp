...
int main(int argc, char** argv)
{
	...
	cout << "准备数据用时：" << duration_cast<milliseconds>(t1 - t0).count() << "ms" << endl;
	...
	cout << "生成模型用时：" << duration_cast<milliseconds>(t2 - t1).count() << "ms" << endl;

	// 将模型导出为xyz格式
	model.saveModel("../../WithoutNormal.xyz");
	
	auto t3 = system_clock::now();
	cout << "Non-Normal用时：" << duration_cast<milliseconds>(t3 - t2).count() << "ms" << endl;

	model.saveModelWithNormal("../../WithNormal.xyz");
	
	auto t4 = system_clock::now();
	cout << "Normal用时：" << duration_cast<milliseconds>(t4 - t3).count() << "ms" << endl;

	system("PoissonRecon.x64 --in ../../WithNormal.xyz --out ../../mesh.ply");

	auto t5 = system_clock::now();
	cout << "泊松重建用时：" << duration_cast<milliseconds>(t5 - t4).count() << "ms" << endl;

	cout << "整体用时：" << duration_cast<milliseconds>(t5 - t0).count() << "ms" << endl;
